...
import com.tokbox.android.accpack.screensharing.util.Base64;

import com.tokbox.android.accpack.screensharing.config.constants.RoleConstants;
import com.tokbox.android.accpack.screensharing.config.constants.SessionProperties;
import com.tokbox.android.accpack.screensharing.util.GenerateMac;
import com.tokbox.android.accpack.screensharing.util.TokBoxXML;
...
public class OpenTokSDK {
     ...
     */
    public String generate_token(String session_id, String role, Long expire_time, String connection_data) throws Exception {
        Long create_time = Long.valueOf(System.currentTimeMillis() / 1000);
        ...
        if(!RoleConstants.SUBSCRIBER.equals(role) &&
                ...
                !"".equals(role))
            throw new Exception(role + " is not a recognized role");

        if(expire_time != null) {
            if(expire_time < (Long.valueOf(System.currentTimeMillis() / 1000 -1)))
                throw new Exception("Expire time must be in the future");
            if(expire_time > (Long.valueOf(System.currentTimeMillis() / 1000 + 2592000)))
                throw new Exception("Expire time must be in the next 30 days");
            data_string_builder.append("&expire_time=");
            data_string_builder.append(expire_time);
        }
        if (connection_data != null) {
            if(connection_data.length() > 1000)
                throw new Exception("Connection data must be less than 1000 characters");
    ...
    public String generate_token(String session_id) throws Exception {
        return this.generate_token(session_id, RoleConstants.PUBLISHER, null, null);
    }


    public String generate_token(String session_id, String role) throws Exception {
        return this.generate_token(session_id, role, null, null);
    }

    public String generate_token(String session_id, String role, Long expire_time) throws Exception {
